...
void test_stringlib()
{
    ...
    std::string output = stringLib.reverseString(input);
    std::cout << "Reverse of '" << input << "' is '" << output << "'\n";
    if (output != "olleh")
        std::cout << "Error: Expected 'olleh', got '" << output << "'\n";

    // Test case 2: Convert to uppercase
    input = "hello world";
    output = stringLib.toUpperCase(input);
    std::cout << "Uppercase of '" << input << "' is '" << output << "'\n";
    if (output != "HELLO WORLD")
        std::cout << "Error: Expected 'HELLO WORLD', got '" << output << "'\n";
}